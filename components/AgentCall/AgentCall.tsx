...
async function InitiatePhoneCall(con_num: string, setCall:React.Dispatch<React.SetStateAction<boolean | null>>) {
  try {
    const response = await fetch("/api/phone-call", {
      ...
      body: JSON.stringify({ con_num: con_num })
    });
...
async function InitiateWebCall(text: string, setIsCallActive: React.Dispatch<React.SetStateAction<boolean>>) {
  try {
...
function AgentCall() {
  ...
  const form = useForm<CallDataType>({
    defaultValues: {
      con_num: ""
    }
  });
  ...
  return (
    <div className="bg-slate-100 grid justify-center items-center p-2 gap-y-7 mb-5">
      ...
      <div className="sm:flex gap-x-2 grid gap-y-2">
        <button
          className="border-2 border-gray-600 p-2 rounded-[5px] font-bold"
      ...
      <div className="pb-5">
        {communication === "phone" ?
          <form onSubmit={handleSubmit((data) => {
            InitiatePhoneCall(data.con_num, setCall);
          })}
            className="grid w-fit m-auto">
            <label
              htmlFor="contuct number"
              className="font-bold">Enter Your Contact Number with Country Code :-</label>
            <input
              type="text"
            ...
            <div className="grid gap-y-2">
              ...
              <div className="flex gap-x-2">
                <button
                  ...
                  onClick={() => InitiateWebCall("test", setIsCallActive)}
